...
const navStyles = tv({
	base: [
		"group peer h-(--navbar-height) w-full items-center px-4 [--navbar-height:3.5rem]",
...
const NavbarNav = ({ className, keepVisible = false, ref, ...props }: NavbarNavProps) => {
	const { isCompact, side, intent, isSticky, open, setOpen } = useNavbar()

	if (isCompact && !keepVisible) {
		return (
